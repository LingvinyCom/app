export default {
	EMAIL_DOMAINS: [
		{
			engine_id: 3,
			domain: 'gmail.com',
			image: require('../assets/img/google.png'),
		},
		{
			engine_id: 2,
			domain: 'exchange',
			image: require('../assets/img/exchange.png'),
		},
		{
			engine_id: 4,
			domain: 'yahoo.com',
			image: require('../assets/img/yahoo.png'),
		},
		{
			engine_id: 1,
			domain: 'icloud.com',
			image: require('../assets/img/icloud.png'),
		},
		{
			engine_id: 6,
			domain: 'outlook.com',
			image: require('../assets/img/outlook-logo.png'),
		},
		{
			engine_id: 5,
			domain: 'aol.com',
			image: require('../assets/img/aol-logo.png'),
		},
		// {
		// 	engine_id: 7,
		// 	domain: 'hotmail.com',
		// 	image: require('../assets/img/hot-mail.jpg'),
		// },
		{
			engine_id: 9,
			domain: 'yandex.ru',
			image: require('../assets/img/yandex-logo.png'),
		},
		{
			engine_id: 8,
			domain: 'mail.ru',
			image: require('../assets/img/mail-ru-logo.png'),
		},
	],
};
